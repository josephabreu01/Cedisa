...
router.get('/estudios/:page', (req, res, next) => {
    ...
    connectionMySql.query({
        sql: 'select * from products pr inner join branchs_has_products bhp using (product_id) inner join inventory_groups using (inventory_group_id) where company_id <> 4 and bhp.branch_id= 1 LIMIT 24',
        timeout: 40000,
...
router.get('/estudio/:id', (req, res, next) => {
    ...
    var medico;
    var querys = {
        estudio: 'select * from products pr inner join branchs_has_products bhp using (product_id) inner join inventory_groups using (inventory_group_id) where company_id <> 4 and bhp.branch_id= 1 and product_id= ' + req.params.id + '  LIMIT 24',
        ars: 'select pr.product_id, pr.product_name,pr.`status`,pr.price1 as precio_privado, ig.name as tipo_estudio, ins.insurance_id as cod_seguro, ins.name as seguro, phi.price as precio_seguro, coverage, pre_autorizacion from products pr inner join branchs_has_products bhp using (product_id) inner join products_has_insurance phi ON phi.product_id = pr.product_id inner join insurance ins ON ins.insurance_id = phi.insurance_id inner join inventory_groups ig ON ig.inventory_group_id = pr.inventory_group_id where company_id <> 4 and bhp.branch_id=' + 1 + ' and pr.status = "A" and is_ars = ' + 1 + ' and pr.product_id= ' + req.params.id + '',
        medico: 'select tec.tech_id, tec.name as medico, br.branch_id, br.name as sucursal, hm.horario from horario hm inner join products pr using (product_id) inner join inventory_groups igt using (inventory_group_id) inner join inventory_groups_has_technician ight using (inventory_group_id) inner join technician tec ON tec.tech_id = ight.tech_id inner join branchs_has_products bhp on bhp.product_id = pr.product_id inner join branchs br ON br.branch_id = hm.branch_id where tec.`status`= ' + 1 + ' and company_id < 4 and bhp.branch_id= ' + 1 + ' and pr.product_id=' + req.params.id + ''
    ...
    async.parallel([
        ...
        arss.forEach(ars => {
            if (ars.pre_autorizacion == '0') {
                ars.pre_autorizacion = "No"
            } else {
                ars.pre_autorizacion = "Si"
            }
        });
        res.json({
            success: true,
